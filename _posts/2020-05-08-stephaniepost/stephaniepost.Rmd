...
#SG: I tried to re-organize the codes after our meeting with Daniel. They all work, we'll just need to be able to describe them for our blog post. Also I found out Method2 and Method3 are the same, DA just assigned a nested function from Method3 into its own function in Method2. There are slight differences in how the output looks between Method 1 & Method 2.  
...
#3 use map_dbl to run a function (e.g. mean) on each num column within tmp
...
# there are a couple of ways to go about returning despcritives looping through the nested list-column dataframe
...
est_one_col(list(mean, sd), tmp$grocery_pharmacy)

desc_df1 <- function(df, funs_to_apply = listof_fun) {
  
  numeric_df <- df[map_lgl(df, is.numeric)] #only select numeric values 

  #Method 1
  map_df(numeric_df, ~est_one_col(funs_to_apply, .x)) %>%
    mutate(column = names(listof_fun)) %>%
...
d2 %>%
  mutate(descrip = map(data, desc_df1)) %>% 
  select(-data) %>% 
  unnest(descrip)


# METHOD 2 - cleaner output 
...
desc_df2 <- function(df, funs_to_apply = listof_fun) {
