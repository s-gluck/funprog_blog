...
But, if you have a bunch of different files, and each is has the same column headers (e.g., files separated by grade), then you could do something like this:

```{r batch-load}
d <- map_df(files, read_csv, .id = "file")
```

In the above code chunk the .id keeps this information from the file name in the first column, as file names often have useful information about the data (e.g., grade level or content area and such).
