...
## Loading the data. 
For details on how to load the data, see [Brock's post](https://s-gluck.github.io/funprog_blog/posts/2020-05-11-blog_post_one/).
For details about this dataset, see [Akhila and Stephanie's post](https://s-gluck.github.io/funprog_blog/posts/2020-05-08-stephaniepost/)
...
### About `pmap()`: pmap(.l, .f, ...)
`pmap()` is a variation of `map()` which allows us to apply a function to multiple vectors simultaneously. According to the [help documentation](https://purrr.tidyverse.org/reference/map2.html), it takes 2 or more arguments: 
...
## Additional arguments to loop through data by country AND category 
...
In order to do so, we'll first have to restructure the data so that mobility category is it's own column. I've done this using `pivot_longer()`: 
...
We can use walk2() to save our plots. It's very similar to map() but doesn't print anything to the screen, instead creates files and file pathways to save. 

```{r}

# add echo = FALSE here once finished!
...
# Question for DA: Any idea why this is not working? The error I get is:  
#Error in grDevices::dev.off() : QuartzBitmap_Output - unable to open file '/Users/joannawright/Desktop/EDLD 610 Spring 2020/funprog_blog/plots/mobility/argentina-grocery-pharmacy.png`
